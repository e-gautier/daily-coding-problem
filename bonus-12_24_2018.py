...
# return deepest
def deepest(node, depth = 1):
  if (node.left is None and node.right is None):
    return (node.value, depth)
  dl = deepest(node.left, depth+1) if node.left else (node, depth)
  dr = deepest(node.right, depth+1) if node.right else (node, depth)
...
def get_depth(node):
  if (node.left is None and node.right is None):
    return 1
  dl = get_depth(node.left) if node.left else 0
  dr = get_depth(node.right) if node.right else 0
  return (dl if dl > dr else dr) + 1
