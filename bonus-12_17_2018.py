...
list = [[10, 15, 30], [12, 15, 20], [17, 20, 32]]
result = list[0]
list.pop(0)
...
for e in range(len(list)):
  for i in range(len(list[e])):
    for index in range(len(result)):
      if (list[e][i] <= result[index]):
        result.insert(index, list[e][i])
        break
      if (index == len(result)-1):
        result.append(list[e][i])
...
heap = [(lst[0], i, 0) for i, lst in enumerate(list) if lst]
heapq.heapify(heap)

while heap:
  element, listIndex, elementIndex = heapq.heappop(heap)
  result.append(element)

  if elementIndex + 1 < len(list[listIndex]):
    nextTuple = (list[listIndex][elementIndex + 1], listIndex, elementIndex + 1)
